...
perform_vectorized_sum_int4(DecompressChunkState *chunk_state, Aggref *aggref)
{
	...
	else if (column_description->type == COMPRESSED_COLUMN)
	{
		...
		while (true)
		{
			...
			//#define USE_CUSTOM_SUM_IMPL
			//#define FORCE_SIMD

#if defined(USE_CUSTOM_SUM_IMPL)
#define SUM_LANES 4

#if defined(FORCE_SIMD)
			/* Int wide data type of 16 byte = 128 bit */
			typedef int v4si __attribute__((vector_size(16)));
			v4si lane_result = { 0, 0, 0, 0 };
			v4si lane_buffer = { 0, 0, 0, 0 };
#else
			/* Use 32 bit values here to be able to fill a 128 bit register with 4 lanes */
			int32 lane_result[] = { 0, 0, 0, 0 };
			int32 lane_buffer[] = { 0, 0, 0, 0 };
#endif
			int cur_lane = 0;
			...
			for (int i = 0; i < arrow->length; i++)
			{
				...
				if (unlikely(arrow_isnull))
					continue;

				lane_buffer[cur_lane] = ((int32 *) arrow->buffers[1])[i];
				cur_lane = (cur_lane + 1) % SUM_LANES;

				if (cur_lane == 0)
				{
#if defined(FORCE_SIMD)
					/* Performs paddd operation on one of xmm registers
					 * (e.g., paddd xmm0, XMMWORD PTR[rbp-0x50])
					 */
					lane_result = lane_result + lane_buffer;
#else

					for (int lane_selector = 0; lane_selector < SUM_LANES; lane_selector++)
					{
						/* Even compiling with -O3 results in 4 add instructions */
						lane_result[lane_selector] += lane_buffer[lane_selector];
					}
#endif
				}
			}
			...
			for (int lane_selector = 0; lane_selector < SUM_LANES; lane_selector++)
			{
				pg_add_s64_overflow(result_sum, lane_result[lane_selector], (int64 *) &result_sum);
			}

			/* If we have in flight results add them too */
			if (cur_lane != 0)
			{
				for (int lane_selector = 0; lane_selector < cur_lane; lane_selector++)
				{
					pg_add_s64_overflow(result_sum,
										lane_buffer[lane_selector],
										(int64 *) &result_sum);
				}
			}
#else
